...
public class ControllerGenerationTests : TestBase
{
    ...
    [Test]
    public void ParameterRequirements_ShouldRespectRequiredAndOptionalModifiers()
    {
        // Arrange
        var openApiFile = Path.Combine(TestDataDirectory, "parameter-requirements-api.yaml");
        var config = CreateDefaultConfiguration();
        ...
        // Assert
        AssertNoErrors(result);
        var controller = GetGeneratedSource(result, "TestController.g.cs");
        ...
        const string expectedController = """
                                          ...
                                          public abstract class TestControllerBase : ControllerBase
                                          {
                                              ...
                                              /// <param name="explicitOptionalQuery"></param>
                                              /// <param name="optionalHeader"></param>
                                              /// <param name="queryWithDefault"></param>
                                              [HttpGet("{requiredPath}")]
                                              public abstract Task<IActionResult> TestRequirements([FromRoute] string requiredPath, [FromQuery] string requiredQuery, [FromHeader] string requiredHeader, [FromQuery] string? optionalQuery, [FromQuery] int? explicitOptionalQuery, [FromHeader] string? optionalHeader, [FromQuery] int queryWithDefault = 10);

                                          }

                                          """;

        AssertSourceEquals(controller.SourceText.ToString(), expectedController, "TestController.g.cs");
    }
}
